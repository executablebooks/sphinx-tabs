...
class TestCase(unittest.TestCase):
    def tearDown(self):  # pylint: disable=invalid-name
        ...
        if StrictVersion(__sphinx_version__) > StrictVersion("1.6.0") and StrictVersion(
            __sphinx_version__
        ) < StrictVersion("1.8.0"):
    ...
    def get_expectation(dirname, filename):
        ...
        if provider.has_resource(resource):
            return pkg_resources.resource_string(__name__, resource).decode("utf-8")
        result = []
        for i in range(10):
            resource_i = "%s.%d" % (resource, i)
            if provider.has_resource(resource_i):
                result.append(
                    pkg_resources.resource_string(__name__, resource_i).decode("utf-8")
                )
        return result

    def assertXMLEqual(self, expected, actual):  # pylint: disable=invalid-name
    ...
    def assertHasTabsAssets(self, xml):  # pylint: disable=invalid-name
        stylesheets = get_stylesheets(xml)
        scripts = get_scripts(xml)

        def filter_scripts(x):
            return (
                x != "documentation_options.js"
                and "mathjax" not in x
                and x != "language_data.js"
            )
    ...
    def assertDoesNotHaveTabsAssets(self, xml):  # pylint: disable=invalid-name
        stylesheets = get_stylesheets(xml)
        scripts = get_scripts(xml)
        for stylesheet in stylesheets:
            assert "sphinx_tabs" not in stylesheet
        for script in scripts:
            assert "sphinx_tabs" not in script

    def assertStylesheetsEqual(self, expected, xml):  # pylint: disable=invalid-name
        actual = get_stylesheets(xml)
        assert expected == actual

    def assertScriptsEqual(self, expected, xml):  # pylint: disable=invalid-name
